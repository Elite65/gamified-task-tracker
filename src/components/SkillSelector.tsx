...
export const SkillSelector: React.FC<SkillSelectorProps> = ({ selectedSkills, onChange }) => {
    ...
    // Get unique existing skills from graph (User's current skills ONLY)
    const userSkillNames = Object.values(userStats.skills || {}).map(s => s.name);

    // Use only user skills, sorted alphabetically
    const existingSkills = [...new Set(userSkillNames)].sort();
